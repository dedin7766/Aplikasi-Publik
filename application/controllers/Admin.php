...
function konfirmasi_act(){
  ...
  if($this->form_validation->run() != false){
    ...
    function anggota(){
      $data['wifi'] = $this->M_kursus->get_data('wifi')->result();
      $this->load->view('admin/header');
      $this->load->view('admin/wifi',$data);
      $this->load->view('admin/footer');
    }
      function hapus_wifi($id){
      $where = array('id' => $id );
      $this->M_kursus->delete_data($where, 'wifi');
      redirect(base_url().'admin/index');
  }

    function tambah_wifi(){
      $this->load->view('admin/header');
      $this->load->view('admin/tambah_wifi');
      $this->load->view('admin/footer');
    }

    function tambah_wifi_act(){
      $nama_wifi = $this->input->post('nama_wifi');
      $status = $this->input->post('status');
      $lokasi = $this->input->post('lokasi');
      $this->form_validation->set_rules('nama_wifi','Nama Anggota','required');
      if($this->form_validation->run() != false){
          $data = array(
            'nama_wifi' => $nama_wifi,
            'status' => $status,
          ...
          $this->M_kursus->insert_data($data,'wifi');
          redirect(base_url().'admin/index');
        }else{
          $this->load->view('admin/header');
      ...
      function update_wifi(){
        ...
        $this->form_validation->set_rules('nama_wifi','Nama wifi','required');
        ...
        if($this->form_validation->run() != false){
            ...
            $this->M_kursus->update_data('wifi',$data,$where);
            redirect(base_url().'admin/index');
          } else{
          }
      }

      function print_data_wifi(){
          $data['wifi'] = $this->M_kursus->get_data('wifi')->result();
          $this->load->view('admin/print_data_wifi',$data);
        }
   ...
   function cari(){
      ...
      $data['wifi']=$this->M_kursus->get_keyword($keyword);
    ...
    public function cari_data()
  {
    ...
    $this->load->view('admin/header');
    $this->load->view('admin/hasil', $data2);
  }
